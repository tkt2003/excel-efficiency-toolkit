...
def test_build_unique_output_path_adds_sequence_without_overwrite(tmp_path):
    first_path = tmp_path / "数据穿透结果.xlsx"
    second_path = tmp_path / "数据穿透结果_2.xlsx"
    first_path.write_text("exists", encoding="utf-8")
    second_path.write_text("exists", encoding="utf-8")

    output_path = build_unique_output_path(str(tmp_path))

    assert output_path == str(tmp_path / "数据穿透结果_3.xlsx")
...
def test_read_drill_cell_from_workbook_marks_empty_value(tmp_path):
    path = tmp_path / "source.xlsx"
    _save_workbook(path, "货币资金", "A1", "占位")
    ...
    assert "缺少指定单元格" in record["message"]
...
def test_write_result_workbook_uses_fixed_headers_without_formula_columns(tmp_path):
    output_path = tmp_path / "数据穿透结果.xlsx"
    records = [
        {
    ...
    assert "公式" not in headers
    assert "是否公式" not in headers
    assert sheet["H2"].value == "是"
    workbook.close()


def _save_workbook(path, sheet_name, cell_address, value):
    workbook = Workbook()
