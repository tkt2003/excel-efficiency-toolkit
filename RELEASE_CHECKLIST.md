...
- `dist` 构建产物名是否为 `老头表格助手.exe`，不能出现中文乱码。
- 最终发布包中的 exe 是否重命名为 `老头表格助手_v1.2.0.exe`。
- exe 能否正常启动。
- 主窗口标题是否显示 `老头表格助手 v1.2.0`。
- 主界面按钮是否正常显示。
- 【使用说明】入口是否能打开。
...
- 建议将 `老头表格助手_v1.2.0.exe` 和 `使用说明.txt` 放在同一个文件夹发布。
...
6. 点击【按行拆分】，确认可选择：
   ...
   - 每 N 行一份
   然后取消。
7. 点击【使用说明】，确认能打开 `使用说明.txt` 或显示内置简版说明。
8. 点击【关于】，确认版本号和阶段说明正确。
9. 确认【按颜色清空内容】按钮仍在。
10. 点击【数据穿透查询】，确认模式选择弹窗能打开，然后取消。
11. 点击【按模板批量生成 Excel】，确认模式选择弹窗能打开，然后取消。
12. 确认日志区正常显示操作记录。
